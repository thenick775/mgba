...
private slots:
	...
	void cbRollbackStart(QList<quint32> list);
	void cbRollbackEnd(QList<quint32> list);
	void updateRooms();
	void updateCores();
...
private:
	...
	static void cbRollbackStart(mNPContext*, const uint32_t* cores, uint32_t nCores, void* user);
	static void cbRollbackEnd(mNPContext*, const uint32_t* cores, uint32_t nCores, void* user);
};
